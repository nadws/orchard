...
public function checkout()
{
    ...
    $bayar = $cash + $bca_kredit + $bca_debit + $mandiri_kredit + $mandiri_debit + $debit + $diskon + $nominal_voucher;
    ...
    $kembali = $bayar - $total;
    $total_semua = $mandiri_debit + $mandiri_kredit + $bca_debit + $bca_kredit + $cash - $kembali;
...
public function edit_pembayaran(){
    ...
    $data = [
        'bayar' => $this->input->post('cash') +  $this->input->post('mandiri_kredit') +  $this->input->post('mandiri_debit') +  $this->input->post('bca_kredit') + $this->input->post('bca_debit'),
        'cash' => $this->input->post('cash'),
        'mandiri_kredit' => $this->input->post('mandiri_kredit'),
        'mandiri_debit' => $this->input->post('mandiri_debit'),
        'bca_kredit' => $this->input->post('bca_kredit'),
...
public function summary_laporan_pemasukan(){
    if (empty($this->input->get('tgl1'))) {
        ...
        $data = array(
            ...
            'servis' => $this->M_salon->summary_servis($dt_a, $dt_b),  
            'penjualan'     => $this->M_salon->summary_penjualan_produk($dt_a, $dt_b),
            'invoice' => $this->db->select_sum('total')->select_sum('diskon')->select_sum('nominal_voucher')->get_where('tb_invoice',['tgl_jam >=' => $dt_a, 'tgl_jam <=' => $dt_b])->row(),
...
public function dt_servis()
{
  $data = array(
      ...
      'bahan' => $this->db->where('id_kategori','13')->get('tb_produk')->result()
  );
...
public function bahan()
{
  $data = array(
      'title'  => "Orchard Beauty", 
      'bahan' => $this->db->join('tb_satuan','tb_produk.id_satuan = tb_satuan.id_satuan','left')->get_where('tb_produk',['id_kategori' => '13'])->result(),
      'satuan' => $this->db->get('tb_satuan')->result()
  );
